"""Session-start header + help text (port of src/cli/banner.ts).
...
from grace.cli.ui.box import box_lines, kv
from grace.cli.ui.theme import symbols, theme
from grace.colors import c
from grace.meta import DISPLAY_NAME, TAGLINE, VERSION


def render_banner(info: dict) -> str:
    th = theme()
    rows = [
    ...
    if info.get("freePlan"):
        rows.append(kv("Quota", info["freePlan"]))
    return "\n".join([
        box_lines([
            "  " + c.bold(c.cyan(DISPLAY_NAME)),
            "  " + c.dim(f"{TAGLINE} · v{VERSION}"),
        ]),
        "",
        *rows,
        "",
...
def render_help() -> str:
    sym = symbols()
    marks = "  " + c.dim(
        sym["bullet"] + " Progress marks: " + sym["check"] + " done · " + sym["cross"] + " failed · " + sym["warn"] + " warning"
    )
    return "\n".join([
        c.bold("Commands"),
